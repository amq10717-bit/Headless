...
// WhatsApp Bubble Style Marquee
const MarqueeRow = ({ items, duration, direction }) => {
  const duplicatedItems = [...items, ...items];

  return (
    <div className="flex w-max overflow-hidden py-2">
      <motion.div
      ...
      >
        {duplicatedItems.map((text, idx) => (
          <div
            key={idx}
            className="px-5 py-2.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl rounded-tr-sm text-sm md:text-base text-pink-100 shadow-lg shadow-black/20 whitespace-nowrap flex items-center"
          >
            {text}
            <span className="ml-2 text-[10px] text-pink-300/60 mt-1 flex items-center">
              ✓✓
            </span>
          </div>
        ))}
...
export default function OpeningPage({ setCurrentPage, setMusicPlaying, setShowMusicPlayer }) {
  ...
  // Function to move the "No" button safely without it falling off screen
  const moveNoButton = useCallback(() => {
    const newX = (Math.random() - 0.5) * 350;
    const newY = (Math.random() * -300) - 50; // Jumps UP strictly

    setNoButtonPos({ x: newX, y: newY })
  }, [])

  useEffect(() => {
    // 1. Generate the 100 Marquee Messages with Emojis
    ...
    // 2. Generate Background Grid Drifting Text
    const cols = 10;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 text-center relative overflow-hidden bg-[#0a0812]">
      ...
      {/* RADIAL MASK CONTAINER - Keeps center clean */}
      <div
      ...
      {/* FOREGROUND CONTENT */}
      <div className="z-20 flex flex-col items-center justify-center w-full max-w-4xl mt-8 relative">
        ...
        >
          ...
          <img
            ...
            className="w-40 h-40 md:w-56 md:h-56 mx-auto rounded-full relative z-10 filter drop-shadow-[0_0_15px_rgba(236,72,153,0.3)]"
          />
          <motion.div
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold drop-shadow-[0_2px_15px_rgba(236,72,153,0.5)] leading-tight tracking-tight relative z-30">
        ...
        {/* THE 2 ROW WHATSAPP MARQUEE */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 5.5, delay: 1.2 }}
          className="w-full max-w-3xl relative z-20 mb-10"
          style={{
        ...
        {/* Buttons Container */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 w-full min-h-[170px] relative z-30">

          {/* Deep dark backdrop to separate buttons from the moving background */}
          <div className="absolute inset-x-0 inset-y-4 bg-black/50 blur-3xl rounded-[100px] pointer-events-none -z-10 scale-125"></div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="px-10 py-5 bg-gradient-to-r from-pink-500/50 via-pink-500/60 to-purple-500/50 backdrop-blur-md border-2 border-pink-300/50 rounded-full text-white text-lg font-bold hover:from-pink-500/60 hover:to-purple-500/60 shadow-[0_10px_30px_rgba(0,0,0,0.6),0_0_25px_rgba(236,72,153,0.4)] transition-all z-30 relative overflow-hidden group"
          >
          ...
          {/* Upgraded "No" Button with Proximity Sensor & Enhanced Shadow */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, x: noButtonPos.x, y: noButtonPos.y === 0 ? 0 : noButtonPos.y }}
            transition={{
              opacity: { duration: 0.8, delay: 1.8 },
              x: { type: "spring", stiffness: 1500, damping: 15 }, // Instant snap
              y: { type: "spring", stiffness: 1500, damping: 15 }
            }}
            onMouseEnter={moveNoButton}
            onClick={moveNoButton}
            className="z-30 p-8 -m-8" // Invisible force field extending 32px around the button
          >
            <button
              className="px-8 py-4 bg-pink-900/40 backdrop-blur-md border border-pink-400/30 rounded-full text-pink-200 text-lg shadow-[0_10px_30px_rgba(0,0,0,0.7)] transition-colors cursor-not-allowed font-medium block"
            >
              Ni krungiii maaf 😤
            </button>
          </motion.div>
